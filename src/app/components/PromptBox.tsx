...
interface PromptBoxProps {
  onSubmit: (image: string | null, prompt: string, isGenerating: boolean) => void;
}
...
export default function PromptBox({ onSubmit }: PromptBoxProps) {
  ...
  const handleGenerate = () => {
    if (promptText.trim()) {
      ...
      setPromptText("");
      onSubmit(null, promptText, true); // Display the prompt text immediately and set isGenerating to true
      ...
      .then(data => {
        onSubmit(imageSrc, "", false); // Update with the generated image and set isGenerating to false
        setImageSrc("");
